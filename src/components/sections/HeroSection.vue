...
function initParticleText(canvas: HTMLCanvasElement, text: string, scatter: boolean = true): { cleanup: () => void; resample: (newText: string) => void } {
  ...
  function sampleText() {
    ...
    offCtx.font = `700 ${fontSize}px "Inter Tight", Arial, sans-serif`
    offCtx.fillStyle = '#fff'
    offCtx.textBaseline = 'middle'
    offCtx.textAlign = 'center'
    ;(offCtx as CanvasRenderingContext2D & { letterSpacing?: string }).letterSpacing = `${Math.round(fontSize * 0.04)}px`
...
onMounted(async () => {
  ...
  if (particleCanvas.value) {
    const scatter = !sessionStorage.getItem('visited')
    const result = initParticleText(particleCanvas.value, t('name'), scatter)
